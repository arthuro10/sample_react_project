...
// Elemente von Semantic UI importieren 
import { Select, Icon, Button, Table, Pagination, Statistic} from 'semantic-ui-react'
...
filterLogs (logs,checkBoxMode,filterFacility,filterSeverity,searchString, searchFromHost,searchSysLogTag,searchMessage) {
  //console.log("filterLogs Aufgerufen!");
  //console.log(logs);
  ...
  return newLogs;
    ...
    render() {
      ...
      const {logsFromServer} = logsStore;
      ...
      const filteredLogs = this.filterLogs(logs,checkBoxMode,filterFacility,filterSeverity,searchString,searchFromHost,searchSysLogTag,searchMessage).slice((globalPagination-1)*this.state.pagePerRow,(globalPagination * this.state.pagePerRow)).map((log,i) =>  
      <Table.Row key={log.ID} onClick={this.showSpecialLog.bind(this,log)}  >
        ...
        const disableSearch = (checkBoxMode.fromHost === false && checkBoxMode.syslogTag === false && checkBoxMode.message === false)  ? false : true
	      ...
	      <div>
              ...
              <Icon  />
              <Icon  />
              <Button onClick={this.reloadClicked.bind(this)} basic color='teal' size='huge' circular >Fetch new Logs!</Button>
